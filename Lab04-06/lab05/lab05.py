...
def distance(city_a, city_b):
    ...
    return sqrt((lat_a - lat_b)**2 + (lon_a - lon_b)**2)#欧式距离
...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_a) if distance_to_a < distance_to_b else get_name(city_b)
    ...
    8
      16
        ...
        5
          10
    """
    if h == 0:  # 如果高度为0，则直接返回n本身
        return [n]
    branches = []  # 初始化树枝列表
    if n > (n ** 0.5) and len(branches) < h - 1:  # 如果当前数字大于n的平方根且树枝数量未达到h-1
        branches += [n // 2, n * 3 + 1]  # 添加左右两个分支
    return hailstone_tree(n // 2, h - 1) if n // 2 != 1 else [] + hailstone_tree(n * 3 + 1, h - 1) if n * 3 + 1 != 1 else []  # 如果n乘以3加1不等于1（即n是奇数），递归子树


def tree(n, branches):
    if not branches:
        return n
    return [n, tree(branches.pop(0), branches)]
