...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        """
        if player.hand:
            copied_card = player.hand[0].copy()
            player.hand.append(copied_card)
            added = True
        else:
            added = False
            self.attack = float('-inf')  # Set power to -inf to ensure losing the round.
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        600 500
        """
        best_card = max(player.hand, key=lambda x: x.power(opponent_card)) if player.hand else None
        if best_card:
